...
- Habits show the current week over a dimmed previous week, keyed to the weekday the user's locale starts on (`Intl.Locale` week info), and any day up to today can be toggled from its own dot. Days that haven't arrived stay inert. Never add a streak counter or score: a number that resets to zero turns a gentle nudge into a source of anxiety.
