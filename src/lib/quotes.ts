...
// Split editor text into one quote per line, preserving what the user typed so editing stays natural.
// Blank lines are cleaned out at display time.
...
// A whole-day number in local time, so the daily quote is stable across reloads on the same calendar day and advances at local midnight.
